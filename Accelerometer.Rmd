...
Segment data
```{r}
fit = function(input_data, cut_size, dup_length=0, num_clusters){
  cutted_signals = list()
  index = 0
  for (observation in input_data){
    index = index + 1
    cutted_signals = c(cutted_signals, cut_signal(observation[-1], cut_size, dup_length))
  }
  matrix_signals = matrix(unlist(cutted_signals), ncol=3*cut_size, byrow=T)
  model = kcca(matrix_signals, k=num_clusters, kccaFamily("kmeans"))
  
  return(model)
}

cut_signal = function(input_data, cut_size, dup_length=0){
  result_data = list()
  ...
  while (input_data_idx + cut_size*3 < input_data_size){
    ...
    input_data_idx = input_data_idx + cut_size*3 - dup_length*3
...
transform = function(model, input_data, cut_size, dup_length, num_clusters){
  features = list()
  index = 0
  for (observation in input_data){
    index = index + 1
    cutted_signals = cut_signal(observation[-1], cut_size, dup_length)
    matrix_signals = matrix(unlist(cutted_signals), ncol=3*cut_size, byrow=T)
...
```{r}
model = fit(data_matrix, 10, 0, 10)
data = transform(model, data_matrix, 10, 0, 10)
