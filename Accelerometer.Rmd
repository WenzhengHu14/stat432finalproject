...
date: "May 9, 2018"
abstract: The following analysis of human activity recognition is based on a data set from the UCI Machine Learning Repository which contains measurements of wrist movement from an accelerometer worn by human test subjects performing a variety of activities of daily living. After preprocessing the data and extracting features via vector quantization, several iterations of four different statistical learning techniques were trained and the leading model of each type was identified. The most accurate model in terms of classification accuracy was the linear support vector machine with 86% accuracy. Development of high accuracy statistical models for classification of human movement data is imperative to improving tools that aid humans' ability to move or understand their movement - from products like Fitbit to rehabilitation devices.
output: 
  html_document: 
    theme: flatly
    toc: true
...
Raw data were collected at 30 hertz by a single triaxial accelerometer attached to each volunteer's right wrist. The ith observation of the raw data contains acceleration measured in the x, y, and z axes which correspond to the following directions of motion:
...
Because KNN is a relatively simple and fast model compared to others used in this analysis, it was possible to test a wide range of parameters, or k-values, before testing. Specifically, parameters k between 3 and 100 (inclusive) were examined.
...
cm_lin_svm_trn = confusionMatrix(lin_trn_pred, data_trn$class)
cm_lin_svm_tst = confusionMatrix(lin_test_pred, data_tst$class)
...
The worst-performing model using k-nearest neighbors achieved an accuracy of 78.86%. This value is considerably lower than most other models which achieved accuracies above 80%. The best model, a linear support vector machine, obtained an accuracy of 86.18% with the following parameters:

- Linear kernel
- Cost (C) parameter of 1000

The confusion matrix for the SVM model can be seen below:

**Linear SVM Confusion Matrix**
```{r echo=FALSE}
cm_lin_svm_tst$table
```
...
The average classification accuracy across all models was 83.17%. However, the most promising system utilized a linear support vector machine that achieved an accuracy of 86.18%. The Linear SVM confusion matrix displayed above highlights the weak points in activity classification. In particular, the model struggles to properly classify activties 9 and 12 (`liedown bed` and `standup chair`). Initially, this was thought be due in part to a low number of instances in the training set. While investigation confirmed this suspicion for `liedown bed` which had only 20 training instances, a total of 72 instances were available for `standup chair`. This is greater than the average number of training instances across all activity types which is 42. This suggests that the difficulty in classification of `standup chair` is not simply due to a lack of training examples, but rather potential similarity between activity types. This makes sense given that when `standup chair` was misclassified, it was predicted to be `sitdown chair` which is a very similar action.

While a positive predictive value of only 33% was achieved training on the 20 examples of activity number 9 (`liedown bed`), it is interesting to note that other classes which achieved fairly good positive prediction with a similar number of training examples (e.g. class 3 `comb hair` achieved a PPV of 100% with only 22 training instances). It is possible that the poor performance on `liedown bed` is simply a result of the minimal number of examples in the testing set (only 8 instances), which allows for easy skewing of performance metrics. Alternatively, it may be that the features provided to the model are insufficient for discriminating this activity type. It would be ideal to obtain additional examples across all activity types to produce a data set which has a balanced class distribution. Upsampling of minority class examples may improve performance, but this approach is not ideal--especially compared to undersampling methods--since upsampling the minority class would require duplication of the current instances which may add unwanted bias to the model.
