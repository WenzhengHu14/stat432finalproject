---
title: 'Classification of Human Wrist Movement'
...
```
The best-performing support vector machine (polynomial) achieved an accuracy of 73.58%. This value is considerably lower than all other models which achieved accuracies above 80%. The best model, a gradient boosted machine, obtained an accuracy of 87.40% with the following parameters: Interaction Depth of 8, 1038 Trees, Shrinkage of 0.01, and 10 Minobsinnode. The confusion matrix for the GBM model can be seen below:
```{r echo=FALSE}
cm_gbm_tst$table
```
...
The average classification accuracy across all models was 78.32%. This is substantially greater than that of all SVM models (average accuracy of 72%). However, the most promising system utilized a gradient boosting machine that achieved an accuracy of 87.40%. The GBM confusion matrix in Figure X highlights the weak points in activity classification. In particular, the model struggles to properly classify activities 13 and 14 (X and Y, respectively). Initially, this was thought be due in part to the low number of instances in the training set. However, inspection of the training set revealed that X and Y have 70 and 72 instances, respectively. This is greater than the average number of instances across all activity types which is 42. This suggests that the difficulty in classification is not simply due to a lack of training examples, but rather potential similarity between activity types.

It should be noted that activity number 9 (XXXX) showed a very poor balanced classification accuracy of 49%. This is quite misleading given that the positive predictive value for that class is in fact 0%. It is interesting that the model failed completely to predict this particular class given that a total of 20 examples were available in the training set. While this is not the largest representative sample, it is on par with some of the other classes which achieved fairly good positive prediction (e.g. class 3 “XXXX” achieved a PPV of 83% with only 22 training instances). It is possible that this poor accuracy is simply a result of the minimal number of examples in the testing set (only 8 instances), which allows for easy skewing of performance metrics. Alternatively, it may be that the features provided to the model are insufficient for discriminating this activity type. It would be ideal to obtain additional examples across all activity types to produce a data set which has a fairly balanced class distribution. Upsampling of minority class examples may improve performance, but this approach is not ideal--especially compared to undersampling methods--since upsampling the minority class would require duplication of the current instances which may add unwanted bias to the model.
